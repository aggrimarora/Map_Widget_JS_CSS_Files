...
var neighbourhood = {
  ...
  "public transport": "https://img1.wsimg.com/isteam/ip/68ccf4ad-ee8d-48fa-8ae2-a2ef5fb22f8a/Transit_Map.png/:/rs=w:1440,h:1440"
};
...
function callback(results, status) {
  check = 0;
  clearInitial();
  // var heading = document.getElementsByClassName("search-heading");
  // heading[0].innerHTML = "Top Results for " + "<i>\"" + selectedIcon + "\"</i>";
  directionRenderer.set('directions', null);
  searchedPlace = null;
  if(infowindow_route != null) infowindow_route.close();
  if(status === google.maps.places.PlacesServiceStatus.OK) {
    for(var i = 0; i < results.length; i++) {
      if(results[i].rating > 3 && distanceTwoPoints(results[i].geometry.location) < 4)
...
function showDetails(placeResult, marker, status) {
  ...
  if (status === google.maps.places.PlacesServiceStatus.OK){
    ...
    //   info = info + "<div style=\"text-align: center\"> <img class=\"picture\" src=" + firstPhoto.getUrl() + " />"
    // }
...
function travelModeChanged(mode) {
    ...
    document.getElementById(mode.id).style.backgroundColor = "#e3e3e3";
    currentMode = mode.id;
    if(mode.id == "driving") {
      selectedMode = google.maps.DirectionsTravelMode.DRIVING;
    }
